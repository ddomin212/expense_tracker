...
const ContentDetails = ({ item, key, type, format, currency }) => {
  const navigate = useNavigate();
  const date = new Date(item?.created || item?.createdAt);
    ...
    <>
      {format === "short" ? (
        ...
        <tr className="align-middle" key={key}>
          ...
          <td className="p-6">{item?.title}</td>
          <td className="p-6">{date.toLocaleDateString("cs-CZ")}</td>
          <td className="p-6">
            {currencyFormat(
              item?.amount,
              currency?.currency || "CZK",
          ...
          <td className="p-6">
            <button
              onClick={() => navigate("/edit", { state: { item, type } })}
