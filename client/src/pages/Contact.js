...
import { contactCreateAction } from "../redux/slices/home/contactSlice";
...
const Contact = () => {
  ...
  const { appErr, serverErr, loading, sent } = contact;
  //formik form
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      dispatch(contactCreateAction(values));
    },
          ...
          <div className="row d-flex justify-content-center">
            <div className="col-md-6 col-xl-4">
              <div>
                ...
                <form className="p-3 p-xl-4" onSubmit={formik.handleSubmit}>
                  <FormItem formik={formik} name={"name"} />
                  <FormItem formik={formik} name={"email"} />
                  <FormItem formik={formik} name={"message"} />
                  <button
                    class={`btn ${
                      sent === true ? "btn-warning disabled" : "btn-primary"
                    } shadow w-100`}
                    type="submit"
                  >
                    {sent === false ? "Sent" : "Send"}
                  </button>
                </form>
              </div>
            </div>
